...
<body>
    ...
    <div class="container-fluid px-4 py-5 my-5 text-center">
        <div class="lc-block mb-4">
            ...
            include("../connection.php");
            $userid=$_SESSION['user_id'];
            $sql="select fname,lname from user_table where userid='$userid'";
            $r=mysqli_query($con,$sql);
            $row=mysqli_fetch_assoc($r);
            $fname=$row['fname'];
            $lname=$row['lname'];
            ...
            ?>
                <!-- <h2 class="display-2 fw-bold">Hello<span class="text-primary"> Ashab Asir!</span></h2> -->
            </div>
        </div>
    ...
    <section class="recipes-section d-flex justify-content-center">
        <div class="recipes-container d-flex lg justify-content-between flex-wrap gap-5">
            ...
            if(isset($_POST['search-btn'])){
                ...
                $sql = "SELECT recipeid, pic, recipename, description FROM recipe_table where status=1 and  recipename LIKE '%$search%' OR description LIKE '%$search%' OR cuisinetype LIKE '%$search%' OR ingredients LIKE '%$search%' OR steps LIKE '%$search%'";
                $r = mysqli_query($con, $sql);
                ...
                if(mysqli_num_rows($r) > 0){
                    while($row = mysqli_fetch_array($r))
                    {
                        ...
                        echo "<a href='user_recipe_details.php?recipeid=$recipeid'class='btn btn-primary'>Read More</a>";
                        echo "</div>";
                        echo "</div>";
                    }
                } else {
            ...
            // Add an else block for displaying all recipes when no search query is provided
            else {
